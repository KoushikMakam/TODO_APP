...
export class TodoRepository {
    ...
    public async create(item: TodoDto, loggedInUser: IUser): Promise<TodoDocument> {
    ...
    public async update(id: string, item: TodoDto, loggedInUser: IUser): Promise<TodoDocument> {
        return await this.todoModel.findOneAndUpdate({ _id: id },
            {
                ...item,
                latsUdpatedDate: Date.now(),
                lastUpdatedBy: loggedInUser
