...
export class TaskRepository {
    ...
    public async findRecordsPassedDate(currentDate: number): Promise<TaskDocument[]> {
        const item = await this.taskModel.find({ 'dueDate': { "$lt": currentDate } }).exec();
        return item;
    }

    public async create(todoDetails: TodoDocument, item: ITask, loggedInUser: IUser): Promise<TaskDocument> {
    ...
    public async update(id: string, item: ITask, loggedInUser: IUser): Promise<TaskDocument> {
        return await this.taskModel.findOneAndUpdate({ _id: id },
            {
                ...item,
                latsUdpatedDate: Date.now(),
                lastUpdatedBy: loggedInUser
