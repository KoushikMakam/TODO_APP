...
import { TaskHistory, TaskHistoryDocument } from "src/mongo/schemas/task_history.schema";
...
export class TaskActivityRepository {
    ...
    constructor(@InjectModel(TaskHistory.name) private taskHistoryModel: Model<TaskHistoryDocument>) {
    }
    ...
    public async create(task: TaskDocument, item: TaskHistoryDto, loggedInUser: IUser): Promise<TaskHistoryDocument> {
    ...
    public async update(id: string, item: TaskHistoryDto): Promise<TaskHistoryDocument> {
        return await this.taskHistoryModel.findOneAndUpdate({ _id: id }, item).exec();
    }
