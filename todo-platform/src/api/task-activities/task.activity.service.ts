...
export class TaskActivityService {
    ...
    public async start(taskId: string, loggedInUser: UserDto): Promise<TaskHistoryDto> {
        ...
        return await this.taskActivityRepository.create(task, taskHistory, loggedInUser)
    }

    public async end(taskId: string, id: string, loggedInUser: UserDto): Promise<TaskHistoryDto> {
        let task = await this.taskRepository.findOne(taskId)
        this.permissionGuard.isTaskActive(task)
