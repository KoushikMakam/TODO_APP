...
export class TaskService {
    ...
    public async create(todoId: string,
        ...
        const savedToDB = await this.taskRepository.create(todoDetails, task, loggedInUser)
        return {
            ...savedToDB["_doc"],
            totalMinutes: 0
        }
    ...
    public async update(todoId: string, id: string,
        request: TaskRequestDto,
        loggedInUser: UserDto,
    ): Promise<TaskResponseDto> {
        const todoDetails = await this.todoRepository.findOne(todoId);
    ...
    public async getByTodo(todoId: string, loggedInUser: UserDto): Promise<TaskResponseDto[]> {
        const todoDetails = await this.todoRepository.findOne(todoId);
        this.permissionGuard.hasPermission(todoDetails, loggedInUser)
    ...
    public async get(todoId: string, id: string, loggedInUser: UserDto): Promise<TaskResponseDto> {
        const todoDetails = await this.todoRepository.findOne(todoId);
        this.permissionGuard.hasPermission(todoDetails, loggedInUser)
        this.permissionGuard.isTodoActive(todoDetails)
    ...
    private async geHoursSpent(record: TaskDocument): Promise<number> {
        let totalMinutes = 0
        const dbActivityRecords = await this.taskActivityRepository.findByTask(record);
        dbActivityRecords.forEach((activity: TaskHistoryDocument) => {
            if (activity.status == TaskStatus.DONE) {
                totalMinutes += new Date(activity.endDate).getMinutes() - new Date(activity.startDate).getMinutes()
            }
            if (activity.status == TaskStatus.IN_PROGRESS) {
                totalMinutes += new Date().getMinutes() - new Date(activity.startDate).getMinutes()
            }
        });
        return totalMinutes;
    }
