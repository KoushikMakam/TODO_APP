...
export class TodoService {
    ...
    public async get(id: string,
        loggedInUser: UserDto): Promise<TodoDto> {
    ...
    public async create(request: TodoRequestDto,
        loggedInUser: UserDto): Promise<TodoDto> {
        const todo: TodoDto = {
            ...request,
            active: true,
    ...
    public async addCoOwners(id: string,
        ...
        try {
            for (const email of coowners.emails) {
                const response: IUser = await this.userServiceClient.send(pattern, {
                    email
                }).toPromise()
